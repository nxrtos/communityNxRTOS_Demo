...
/*
 * A sample implementation of pvPortMalloc() and vPortFree() that to implement
 * a scheme of fixed pre-sized memory blocks in mem_pool and does not further
 * break down or merge.
 *
 * presize performed at prvHeapInit.
 *
 * Manage Fixed Size Heap
 *
 * ref book.
 * << Dynamic Memory Management for Embedded Systems >>
 * By David Atienza Alonso, Stylianos Mamagkakis, Christophe Poucet, Miguel Peón-Quirós, Alexandros Bartzas, Francky Catthoor, Dimitrios Soudris
 *
...
void *pvAlgorithmPrePartition_Malloc( const MemPoolConfig_t * const pMemPoolConfig,size_t xWantedSize )
{
...
void AlgorithmPrePartition_Free(const MemPoolConfig_t * const pMemPoolConfig, void *pv )
{
			...
			if( pxLink->pxNextFreeBlock == NULL )
			{
                ...
                else
                {
                    *(pMemPoolConfig->mem_pool_op_lock) = 0x01;
                    //continue op on mem_pool_op_priviledge
                }
...
// MemPool Init with pre-partition action
